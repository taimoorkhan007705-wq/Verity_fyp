import fs from "fs"
import path from "path"
import dns from "dns"
import { exec } from "child_process"
import { promisify } from "util"
import mongoose from "mongoose"
import dotenv from "dotenv"

dotenv.config()

const envPath = path.join(process.cwd(), '.env')
const envText = fs.readFileSync(envPath, 'utf8')
console.log('Loaded .env length:', envText.length)
console.log('MONGODB_URI raw starts:', process.env.MONGODB_URI?.slice(0, 30))

const parseSrvUri = (uri) => {
  try {
    const url = new URL(uri)
    if (url.protocol !== 'mongodb+srv:') return null
    return {
      username: url.username ? encodeURIComponent(url.username) : undefined,
      password: url.password ? encodeURIComponent(url.password) : undefined,
      host: url.hostname,
      pathname: url.pathname,
      search: url.search
    }
  } catch (err) {
    return null
  }
}

const resolveSrvHosts = async (clusterHost) => {
  try {
    const records = await dns.promises.resolveSrv(`_mongodb._tcp.${clusterHost}`)
    return records.map(r => ({ host: r.name, port: r.port }))
  } catch (firstError) {
    const command = `nslookup -type=SRV _mongodb._tcp.${clusterHost}`
    const { stdout } = await promisify(exec)(command, { timeout: 5000 })
    const lines = stdout.split(/\r?\n/)
    const hosts = []
    let current = {}
    for (const line of lines) {
      const trimmed = line.trim()
      if (trimmed.startsWith('port')) {
        const portMatch = trimmed.match(/port\s*=\s*(\d+)/i)
        if (portMatch) current.port = parseInt(portMatch[1], 10)
      }
      if (trimmed.toLowerCase().includes('svr hostname')) {
        const hostMatch = trimmed.match(/svr hostname\s*=\s*(.+)$/i)
        if (hostMatch) current.host = hostMatch[1].trim()
      }
      if (current.host && current.port) { hosts.push(current); current = {} }
    }
    return hosts
  }
}

const buildStandardMongoUri = (srvUri, hosts) => {
  const parsed = parseSrvUri(srvUri)
  if (!parsed) return null
  const authPart = parsed.username ? `${parsed.username}${parsed.password ? `:${parsed.password}` : ''}@` : ''
  const hostPart = hosts.map(({ host, port }) => `${host}:${port}`).join(',')
  const searchParams = new URLSearchParams(parsed.search)
  if (!searchParams.has('tls') && !searchParams.has('ssl')) {
    searchParams.set('tls', 'true')
  }
  return `mongodb://${authPart}${hostPart}${parsed.pathname}${searchParams.toString() ? `?${searchParams.toString()}` : ''}`
}

const uri = process.env.MONGODB_URI
if (!uri) { console.error('No MONGODB_URI present'); process.exit(1) }
const parsed = parseSrvUri(uri)
console.log('Parsed SRV URI:', parsed ? parsed.host : 'not srV')

const tryConnect = async (connUri) => {
  console.log('Trying connect to', connUri.startsWith('mongodb+srv://') ? 'SRV uri' : 'standard uri')
  await mongoose.connect(connUri, { serverSelectionTimeoutMS: 10000, connectTimeoutMS: 10000, socketTimeoutMS: 10000, family:4 })
  console.log('Connected successfully!')
  await mongoose.disconnect()
}

const run = async () => {
  try {
    if (parsed) {
      try {
        await tryConnect(uri)
        return
      } catch (e) {
        console.error('SRV connect failed:', e.message)
      }
      try {
        const hosts = await resolveSrvHosts(parsed.host)
        console.log('Resolved hosts:', hosts)
        const fallback = buildStandardMongoUri(uri, hosts)
        console.log('Fallback URI built:', fallback ? fallback.slice(0,30) : 'none')
        if (fallback) {
          await tryConnect(fallback)
          return
        }
      } catch (e) {
        console.error('Fallback resolve/connect failed:', e.message)
      }
      console.error('All connection attempts failed')
    } else {
      await tryConnect(uri)
    }
  } catch (e) {
    console.error('Final error:', e)
  }
}
run()
